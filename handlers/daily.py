...
# WORK KOMUTU - basit test versiyonu
@router.message(Command(commands=["work"]))
async def work_command(message: Message):
    print("WORK komutu yakalandi!")  # DEBUG
    await message.answer("Work komutu calisiyor! /work 3.5 yazmalisin.", reply_markup=main_menu())

# SORU KOMUTU
@router.message(Command(commands=["soru"]))
async def question_command(message: Message):
    print("SORU komutu yakalandi!")  # DEBUG
    await message.answer("Soru komutu calisiyor! /soru 120 yazmalisin.", reply_markup=main_menu())

# Detayli work komutu (parametreli)
@router.message(Command(commands=["work"]))
async def work_command_detailed(message: Message):
    args = message.text.split()
    user_id = message.from_user.id
    
    print(f"WORK DETAIL: {message.text}")  # DEBUG
...
# Detayli soru komutu
@router.message(Command(commands=["soru"]))
async def question_command_detailed(message: Message):
    args = message.text.split()
    user_id = message.from_user.id
    
    print(f"SORU DETAIL: {message.text}")  # DEBUG
...
async def daily_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        "Asagidaki komutlari kullanarak giris yapabilirsin:\n\n"
        "/work 3.5 - 3.5 saat calistim\n"
        "/work 3.5 120 - 3.5 saat calistim, 120 soru cozdum\n"
